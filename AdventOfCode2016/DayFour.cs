...
namespace AdventOfCode2016
{
    ...
    {
        public static string PUZZLE_INPUT = @"aczupnetwp-mfyyj-opalcexpye-977[peyac]
...
bxaxipgn-vgpst-qphzti-rdcipxcbtci-635[ipctx]";
    ...
    // Object that holds data related to rooms encrypted in the Bunny HQ database
    public class EncryptedRoom
    {
        // Dictionary of characters and their number of occurance in the encrypted room name
        public Dictionary<char, int> letterCount;

        // Rooms sectorId
        public int sectorId;

        // Checksum used to determine if the room is "real"
        ...
        {
            List<string> splitCheckSum = input.Split('[').ToList<string>();
            checksum = splitCheckSum[1].Substring(0, splitCheckSum[1].Count() - 1);

            List<string> splitBodyString = splitCheckSum[0].Split('-').ToList<string>();
            string sectorIdString = splitBodyString[splitBodyString.Count - 1];
            sectorId = Int32.Parse(sectorIdString);

            // Handle sorted list of characters
            int sizeSectorId = sectorIdString.Count();
            string encryptedName = splitCheckSum[0].Substring(0, splitCheckSum[0].Length - (sizeSectorId + 1));
            string removedDashes = Regex.Replace(encryptedName, @"\-", "");

            letterCount = new Dictionary<char, int>();

            List<char> characters = removedDashes.ToList<char>();
            foreach (char character in characters)
            {
                if (!letterCount.ContainsKey(character))
                {
                    int numberOfChar = removedDashes.Count(c => c == character);
                    letterCount.Add(character, numberOfChar);
                }
            }

            // Determine the decrypted name
            string decrypted = "";
            foreach (char character in encryptedName.ToList<char>())
            {
                decrypted += DecryptChar(character, sectorId);
            }
            decryptedName = decrypted;
        }

        // A room is "real" if its checksum matches the five most common characters, in order, in the encrypted name
        public bool IsReal()
        {
            // Sort the Dictionary
            ...
            string output = "";
            List<char> keys = (from letters in listOfLetters select letters.Key).ToList();
        ...
        // If we hit "z" then wrap around to "a"
        private char DecryptChar(char input, int sectorId)
        {
            if (input == '-')
                return ' ';

            char result = input;
            // Convert to ascii to handle the increase in characters - ascii a = 97; z = 122
            for (int i = 1; i <= sectorId; i++)
            {
                int ascii = (int)result + 1;
